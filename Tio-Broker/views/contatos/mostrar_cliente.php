...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        ...
        <div class="flex items-center space-x-2">
            ...
            <!-- CORREÇÃO: Botão Excluir adicionado -->
            <a href="<?= BASE_URL ?>views/contatos/index.php?controller=cliente&action=excluir&id_cliente=<?= htmlspecialchars($cliente['id_cliente']) ?>" class="bg-red-600 hover:bg-red-700 text-white text-sm font-medium py-2 px-4 rounded-lg shadow-sm flex items-center btn-excluir-cliente">
...
document.addEventListener('DOMContentLoaded', function () {
    // Script para as abas de formulários
    const tabs = [];
    const tabContents = {};
    ...
    // CORREÇÃO: Script para o modal de confirmação de exclusão do cliente
    const btnExcluirCliente = document.querySelector('.btn-excluir-cliente');
    if (btnExcluirCliente) {
        btnExcluirCliente.addEventListener('click', function(event) {
            event.preventDefault();
            const urlParaExcluir = this.href;
            const nomeCliente = "<?= addslashes(htmlspecialchars($cliente['nome'])) ?>";

            Swal.fire({
                title: 'Você tem certeza?',
                html: `Deseja realmente excluir o cliente <strong>${nomeCliente}</strong>?<br><small>Esta ação não poderá ser revertida!</small>`,
                icon: 'warning',
