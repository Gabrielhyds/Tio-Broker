...
<div class="crm-card">
    ...
    <div class="crm-header">
        ...
        <div class="flex flex-col lg:flex-row items-stretch lg:items-center gap-4 w-full lg:w-auto">
            ...
            <form method="GET" action="listar.php" class="flex items-center gap-2">
                <div class="relative">
                    <i class="bi bi-search absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"></i>
                    <input type="text" name="filtro" class="w-full md:w-64 border border-gray-300 rounded-md pl-10 pr-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Buscar por nome, email..." value="<?= htmlspecialchars($filtro ?? '') ?>">
                </div>
                <?php if (!empty($filtro)): ?>
        ...
        <table class="min-w-full bg-white text-sm text-left text-gray-700">
            <thead class="bg-gray-50 text-xs uppercase font-semibold text-gray-600">
                <tr>
                    ...
                    <th class="px-6 py-3">Permissão</th>
                    <th class="px-6 py-3">Imobiliária</th>
            ...
            <tbody class="divide-y divide-gray-200">
                <?php if (!empty($lista)): ?>
                    <?php foreach ($lista as $u): ?>
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 font-medium text-gray-900"><?= htmlspecialchars($u['nome']) ?></td>
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="5" class="px-6 py-10 text-center text-gray-400">Nenhum usuário encontrado.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
